...
void SSL_Flush(FESLCtx* ctx) {
	...
	if (ssl_write_sz > 0) {
		...
		fprintf(console_fd, "write send: %d\n", r);
	}
	int ssl_read_sz = BIO_pending(g_read_bio);
	if (ssl_read_sz > 0) {
		BIO_read(g_read_bio, &ctx->ssl_state->send_buffer, ssl_write_sz);

		int r = send(ctx->fesl_socket->socket, (const char*)&ctx->ssl_state->send_buffer, ssl_write_sz, 0);
		fprintf(console_fd, "read send: %d\n", r);
	}
}
void SSL_Read(FESLCtx* ctx) {
	...
	if (r < 0) {
		fprintf(console_fd, "recv error: %d\n", r);
		...
		if (!SSL_is_init_finished(g_ssl)) {
			...
			fprintf(console_fd, "INIT FINISHED, DO SSL READ\n");
		}
	}
}
...
int SSL_LogicThread(FESLCtx* ctx) {
	...
	if (ctx->connection_state == 2) {
		...
		while (true) {
			...
			fprintf(console_fd, "RECV: %d\n", r);
			if (r <= 0) {
				break;
			}
			BIO_write(g_read_bio, (char*)&recvbuf[0], r);
			fprintf(console_fd, "recv bio write: %d\n", r);
		}
...
int fesl_SSL_recv(FESLCtx* ctx, char* buf, int len) {
	fprintf(console_fd, "fesl_SSL_recv - %d\n", len);
	...
	if (ctx->connection_state == 30) {
		...
		if (ctx->ssl_state->recv_buffer_cursor >= ctx->ssl_state->recv_expected_len) {
			...
			fprintf(console_fd, "did reset: %d\n", read_len);
		}
		result = read_len;
		show_dump((unsigned char *)buf, read_len, console_fd);
		
	}
	fprintf(console_fd, "RECV RESULT - %d\n", result);
...
void install_fesl_patches() {
	...
	SSL_CTX_set_verify(g_ssl_ctx, SSL_VERIFY_PEER, verify_callback);
